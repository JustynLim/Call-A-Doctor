...
# Global variable(s)
path_db_file    = "Python\Call-A-Doctor\db\database.db"

class MySideBar(QtWidgets.QMainWindow, Ui_MainWindow):
    __instance = None
# class MySideBar(QMainWindow, Ui_MainWindow):
    def __new__(cls, doctor_email, parent = None):
    ...
    def __init__(self, doctor_email, parent = None):
        if not hasattr(self, 'ui'):
            ...
            self.doctor_email = doctor_email  # Store doctor's email
        ...
        # Populate doctor's profile information
        self.populate_profile_info()

        cursor.execute("SELECT DOCTOR_NAME, CLINIC_NAME, DOCTOR_ADDRESS, DOCTOR_PHONENO FROM DOCTORS WHERE DOCTOR_EMAIL = ?",(self.doctor_email,))
        profile = cursor.fetchall()
        conn.close()

        #insert into the respective labels in profile page
        self.label_username_fill.setText(profile[0][0])
        self.label_clinicName_fill.setText(profile[0][1])
        self.label_address_fill.setText(profile[0][2])
        self.label_phoneNumber_fill.setText(profile[0][3])
        ...
        self.ui.logout_btn_1.clicked.connect(self.logout)
        self.ui.logout_btn_2.clicked.connect(self.logout)
    ...
    def load_appointments(self):
        try:
            ...
            cursor.execute(
                "SELECT APPOINTMENT_ID, APPOINTMENT_DATE, APPOINTMENT_TIME, PATIENT_NAME FROM APPOINTMENT WHERE APPOINTMENT_DATE = ? AND DOCTOR_NAME = (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?)",
    ...
    def highlight_appointment_dates(self):
        try:
            ...
            cursor.execute("SELECT DISTINCT APPOINTMENT_DATE FROM APPOINTMENT WHERE DOCTOR_NAME = (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?)",
            (self.doctor_email,))
    ...
    def show_appointments(self, date):
        try:
            ...
            cursor.execute(
                "SELECT APPOINTMENT_ID, APPOINTMENT_DATE, APPOINTMENT_TIME, PATIENT_NAME, DOCTOR_NAME FROM APPOINTMENT WHERE APPOINTMENT_DATE = ? AND DOCTOR_NAME = (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?)",
    ...
    def load_approvals(self):
        try:
            ...
            cursor.execute("""
                SELECT DISTINCT APPROVAL_ID, DOCTOR_NAME_SEND, PATIENT_NAME, RECORD_ID
                FROM PATIENT_RECORD_APPROVAL
                WHERE DOCTOR_NAME_RECEIVED = (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?)
                AND STATUS = 0
            """, (self.doctor_email,))
        ...
        try:
            ...
            with sqlite3.connect(path_db_file) as conn:
                ...
                cursor.execute("INSERT INTO PATIENT_RECORD (PATIENT_NAME, DOCTOR_NAME, DIAGNOSIS, PRESCRIPTION, RECORD_DATE) VALUES (?, ?, ?, ?, ?)",
                            (patient_name, doctor_name, diagnosis, prescription, record_date))
    ...
    def search_patient_records(self):
        ...
        try:
            ...
            FROM PATIENT_RECORD_APPROVAL
            WHERE DOCTOR_NAME_SEND = (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?)
            GROUP BY RECORD_ID
            """

            # Main query to get patient records with necessary joins
            query = f"""
            SELECT pr.RECORD_ID, pr.PATIENT_NAME, pr.DOCTOR_NAME, pr.RECORD_DATE,
            ...
            WHERE pr.PATIENT_NAME LIKE ?
            """
            
            # Execute the main query with the required parameters
            cursor.execute(query, (self.doctor_email, '%' + search_name + '%'))
            ...
            if reply == QMessageBox.Yes:
                try:
                    ...
                    cursor.execute(
                        ...
                        (DOCTOR_NAME_RECEIVED, PATIENT_NAME, RECORD_ID, STATUS, DOCTOR_NAME_SEND) 
                        VALUES 
                        ((SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_NAME = ?), 
                        (SELECT PATIENT_NAME FROM PATIENT_RECORD WHERE RECORD_ID = ?), 
                        ?, 0, 
                        (SELECT DOCTOR_NAME FROM DOCTORS WHERE DOCTOR_EMAIL = ?))
                        """, 
                        (doctor_name, record_id, record_id, self.doctor_email)
                    )
