...
class Login_Form(QtWidgets.QMainWindow):
    login_successful = QtCore.Signal(bool)  # Signal for login status
    ...
    def toggle_password_visibility(self, state):
        if state == QtCore.Qt.CheckState.Checked:
            self.lineEdit_2.setEchoMode(QtWidgets.QLineEdit.EchoMode.Normal)  # Display password in plaintext format
        else:
            self.lineEdit_2.setEchoMode(QtWidgets.QLineEdit.EchoMode.Password)  # Display password using '*'
    ...
    def signin(self):
        global email_input1, password_input1
        email_input1 = self.lineEdit.text().lower()
        password_input1 = self.lineEdit_2.text()
        ...
        if email_input1 and password_input1:
            ...
            if login_result:
                ...
                self.login_successful.emit(True)  # Emit signal on successful login
            else:
                login_display_status(self.parent(), 0)
                self.login_successful.emit(False)  # Emit signal on failed login
        else:
            login_display_status(self.parent(), 0)
            self.login_successful.emit(False)  # Emit signal on failed login
