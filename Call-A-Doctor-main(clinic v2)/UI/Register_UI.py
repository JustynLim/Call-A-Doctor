...
class Register_Form(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        email_input2 = None
        password_input2 = None
        confirm_password_input = None
        activation_code_input = None
    ...
    def setupUi(self, Form):
        ...
        self.checkBox_2.setStyleSheet("QCheckBox::indicator:checked{image: url(:/resources/hide_password.png);}\n"
        ...
        self.checkBox_3.setStyleSheet("QCheckBox::indicator:checked{image: url(:/resources/hide_password.png);}\n"
                                      "QCheckBox::indicator:unchecked{image: url(:/resources/show_password.png);}")
    ...
    def toggle_password_visibility(self, state, lineEdit):
        if state == QtCore.Qt.CheckState.Checked:
            lineEdit.setEchoMode(QtWidgets.QLineEdit.EchoMode.Normal)  # Display password in plaintext format
        else:
            lineEdit.setEchoMode(QtWidgets.QLineEdit.EchoMode.Password)  # Display password using '*'
    ...
    def signup(self):
        email_input2 = self.lineEdit_5.text().lower()
        password_input2 = self.lineEdit_6.text()
        confirm_password_input = self.lineEdit_7.text()
        activation_code_input = self.lineEdit_8.text()

        status = -1

        if not email_input2:
            status = 0
        elif not password_input2 or not confirm_password_input:
            status = 1
        elif password_input2 != confirm_password_input:
            status = 2
        elif not activation_code_input:
            status = 4
        ...
        status = register(email_input2, password_input2, activation_code_input)
        register_display_status(self, status)

        if status == 6:  # Successful registration
