...
from PySide6.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QMessageBox, QCompleter
from PySide6.QtCore import QDate, Qt
from PySide6.QtGui import QPainter, QPalette, QColor
from .ui_doctor import Ui_MainWindow
from DoctorLoginController import LoginController
...
class MySideBar(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def highlight_appointment_dates(self):
        try:
            ...
            # Debug: Print converted QDates
            print("Converted QDates:", self.appointment_dates)
            ...
            # Debug: Confirm paintCell method override
            print("Custom paintCell method set")
        except sqlite3.Error as e:
            print(f"SQLite error: {e}")
    ...
    def create_messagebox(self, title, text, icon, buttons = QMessageBox.StandardButtons(QMessageBox.Ok)):
    ...
    def load_approvals(self):
        try:
            ...
            self.ui.list_approval.setRowCount(0)
            ...
            msg_box = self.create_messagebox("Error", f"Database error: {e}", QMessageBox.Icon.Critical)
            msg_box.exec()
    ...
    def approve_selected(self):
        try:
            ...
            if not selected_rows:
                ...
                msg_box = self.create_messagebox("No Selection", "No approval selected", QMessageBox.Icon.Warning)
                msg_box.exec()
                return
            ...
            with sqlite3.connect(path_db_file) as conn:
                ...
                msg_box = self.create_messagebox("Success", "Selected approvals have been approved", QMessageBox.Icon.Information)
                msg_box.exec()
    ...
    def reject_selected(self):
        try:
            ...
            with sqlite3.connect(path_db_file) as conn:
                ...
                msg_box = self.create_messagebox("Success", "Selected approvals have been rejected", QMessageBox.Icon.Information)
                msg_box.exec()
    ...
    def save_patient_record(self):
        ...
        if not (patient_name and diagnosis and prescription):
            ...
            msg_box = self.create_messagebox("Missing Information", "Please fill in all fields", QMessageBox.Icon.Warning)
            msg_box.exec()
            return
        ...
        try:
            ...
            with sqlite3.connect(path_db_file) as conn:
                ...
                msg_box = self.create_messagebox("Success", "Patient record saved successfully.", QMessageBox.Icon.Information)
                msg_box.exec()
            ...
            msg_box = self.create_messagebox("Error", f"Failed to save patient record: {e}", QMessageBox.Icon.Critical)
            msg_box.exec()
    ...
    def search_patient_records(self):
        ...
        if not search_name:
            ...
            msg_box = self.create_messagebox("Missing Information", f"Please enter a patient name to search", QMessageBox.Icon.Warning)
            msg_box.exec()
            return
        ...
        try:
            ...
            if not patient_email:
                ...
                msg_box = self.create_messagebox("No Results", "Patient name not found", QMessageBox.Icon.Warning)
                msg_box.exec()
                conn.close()
                return
            ...
            if not records:
                ...
                msg_box = self.create_messagebox("No Results", "No records found for the entered patient name", QMessageBox.Icon.Information)
                msg_box.exec()
    ...
    def display_record_details(self):
        ...
        if selected_row == -1:
            ...
            msg_box = self.create_messagebox("No Selection", "Please select a record to view details.", QMessageBox.Icon.Warning)
            msg_box.exec()
            return
        ...
        if not record_id_item or not doctor_name_item or not access_status_item:
            ...
            msg_box = self.create_messagebox("Selection Error", "Selected row is invalid.", QMessageBox.Icon.Warning)
            msg_box.exec()
            return
        ...
        if access_status == "accessible":
            try:
                ...
                if record:
                    ...
                    msg_box = self.create_messagebox("Record Error", "Record not found in the database.", QMessageBox.Icon.Warning)
                    msg_box.exec()
            ...
            msg_box = self.create_messagebox('Approval Request', 
                                             f"Do you want to send an approval request to Dr. {doctor_name} to access this record?", 
                                             QMessageBox.Icon.Question, QMessageBox.StandardButtons(QMessageBox.Yes | QMessageBox.No))
            reply = msg_box.exec()
            if reply == QMessageBox.Yes:
                try:
                    ...
                    msg_box = self.create_messagebox("Request Sent", f"Approval request sent to Dr. {doctor_name}.", QMessageBox.Icon.Information)
                    msg_box.exec()
                
                except sqlite3.Error as e:
                    print(f"SQLite error: {e}")
                    #QMessageBox.critical(self, "Database Error", f"Failed to send approval request: {e}")
                    msg_box = self.create_messagebox("Database Error", f"Failed to send approval request: {e}", QMessageBox.Icon.Critical)
                    msg_box.exec()
